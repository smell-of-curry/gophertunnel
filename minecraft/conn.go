...
func newConn(netConn net.Conn, key *ecdsa.PrivateKey, log *logrus.Logger, proto Protocol, flushRate time.Duration, limits bool) *Conn {
	conn := &Conn{
		...
		conn:         netConn,
		privateKey:   key,
		log:          log.WithField("raddr", netConn.RemoteAddr().String()).Logger,
		hdr:          &packet.Header{},
		proto:        proto,
		readerLimits: limits,
	}
	...
	for _, pack := range pk.TexturePacks {
		for i, behaviorPack := range pk.BehaviorPacks {
			if pack.UUID == behaviorPack.UUID {
				// We had a behavior pack with the same UUID as the texture pack, so we drop the behavior
				// pack and log it.
				conn.log.Warn("handle ResourcePackStack: dropping behavior pack due to a texture pack with the same UUID", "UUID", pack.UUID)
